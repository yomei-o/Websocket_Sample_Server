...
#if (defined(unix) && !defined(__CYGWIN__)) || defined(__APPLE__)
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <signal.h>
...
int myfifo_ismessage(void* vp)
{
	...
	if (vp == NULL)return -1;
	r = GetMailslotInfo(vp, NULL, NULL, &ct, &to);
	if (r && ct)ret = 1;
#endif
#if (defined(unix) && !defined(__CYGWIN__)) || defined(__APPLE__)
	int  r;
...
void doread(void)
{
	...
	for(i=0;i<30;i++){
		printf("reading...\n");
		memset(buf,0,256);
		if (myfifo_ismessage(h) == 0){
			printf("ismessage is null...\n");
			myfifo_sleep(1);
			continue;
		}
